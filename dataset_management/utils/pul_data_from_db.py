...
def get_data_from_db(oc, sev, snr, db_name, filter_to_use):
    ...
    if len(healthy_data) == 0:  # Skip if there is no data
        print(
            "No healthy data found in the database for operating condition oc: "
            + str(oc)
            + " snr: "
            + str(snr)
            + " sev: "
    ...
    all_modes = collection.distinct(
        "mode", filter={"oc": oc, "severity": sev, "snr": snr}
    ...
    for fault_mode in faulty_modes:
        ...
        faulty_test_datasets[fault_mode] = list(
            collection.find(
                {
                    "oc": oc,
                    "severity": sev,
        ...
        if sum([len(val) for key, val in faulty_test_datasets.items()]) == 0:
            print(
                "No faulty data for any fault mode found in the database for operating condition oc: "
                + str(oc)
                + " snr: "
                + str(snr)
                + " sev: "
    ...
    if "fft" in filter_to_use.keys():
        ...
        for key, faulty_test_data in faulty_test_datasets.items():
            faulty_test_datasets[key] = pd.DataFrame(
                [d["fft"] for d in faulty_test_data]
            )
    elif "time_series" in filter_to_use.keys():
        healthy_data = pd.DataFrame([d["time_series"] for d in healthy_data])
        for key, faulty_test_data in faulty_test_datasets.items():
            faulty_test_datasets[key] = pd.DataFrame(
                [d["time_series"] for d in faulty_test_data]
            )
    else:
...
def get_data_from_db_and_save_to_file(db_name):
    ...
    engineering_filter = {
        key: 1 for key in engineering_features_to_use
    }  # The filter that will extract only the relevant fields from the database
    ...
    freqs = np.fft.fftfreq(
        db_metadata["cut_signal_length"], d=1 / db_metadata["sampling_frequency"]
    )
    freqs = freqs[
        1 : db_metadata["cut_signal_length"] // 4
    ]  # Only use half of the the positive frequencies and remove the DC component
    ...
    prod = product(
        collection.distinct("oc"),
        severity_excluding_healthy,
        collection.distinct("snr"),
    )
    for oc, sev, snr in prod:
        ...
        for filter_to_use, data_type in zip([time_series_filter], ["time_series"]):
            healthy, faulty_data_dict, faulty_test_data_expected_fault_freqs = (
                get_data_from_db(oc, sev, snr, db_name, filter_to_use)
            )

            if healthy is not None:
                print(
                    "for OC: {} SNR: {} SEV: {}, lengths are: healthy {}, faulty {}".format(
                        oc,
                        snr,
                        sev,
                        len(healthy),
                        [len(faulty) for faulty in faulty_data_dict.values()],
                    )
                )
                meta_data = {}

                # Calculate the ground truth fault direction
                ground_truth_fault_dir = (
                    pd.concat(list(faulty_data_dict.values()), ignore_index=True).mean()
                    - healthy.mean()
                )
                ...
                if data_type == "engineering":
                    meta_data.update(
                        {
                            "expected_fault_direction": list(
                                np.ones(len(ground_truth_fault_dir))
                            )
                        }
                    )  # Assumption that all engineering features are equally important and are increasing with fault severity
                elif data_type == "frequency":
                    cumulative_expected_fault_direction = np.zeros(
                        len(ground_truth_fault_dir)
                    )
                    for (
                        mode,
                        expected_fault_freq,
                    ) in faulty_test_data_expected_fault_freqs.items():
                        expected_fault_direction_for_mode = (
                            peak_simulator.get_expected_fault_behaviour(
                                1, expected_fault_freq
                            )
                        )
                        meta_data.update(
                            {
                                "expected_fault_direction_" + mode: list(
                                    expected_fault_direction_for_mode
                                )
                            }
                        )
                        cumulative_expected_fault_direction += (
                            expected_fault_direction_for_mode
                        )
                    meta_data.update(
                        {
