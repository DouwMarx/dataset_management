...
from dataset_management.utils.update_database import DerivedDoc
...
def process(
    db_name, doc, median_variance, snr_levels=None
):  # TODO: Automate median variance computation
    ...
    for sd, snr in zip(noise_sd, snr_levels):
        ...
        new_doc["time_series"] = list(
            time_series + np.random.normal(0, sd, size=time_series.size)
        )  # Notice numpy takes the standard deviation as argument
        docs.append(new_doc)
...
def add_noisy_copies_of_dataset(dataset_name, snr_levels=None, parallel=True):
    ...
    if parallel:
        Parallel(n_jobs=10)(
            delayed(process)(dataset_name, doc, median_variance, snr_levels)
            for doc in tqdm(list(db["raw"].find({"snr": 0})))
        )
    else:
...
def main(db_name):
    add_noisy_copies_of_dataset(
        db_name, snr_levels=np.logspace(-2, 0, 3), parallel=True
    )
