...
class TriangularPeaks:
    ...
    def __init__(self, freqs_to_simulate_for, alpha=0.001, traingle_base=50):
        self.alpha = alpha
        self.width = traingle_base

        self.freqs_to_simulate_for = freqs_to_simulate_for

        self.augmentation_meta_data = {
            "triangle_base": traingle_base,
            "alpha": alpha,
        }
    ...
    def get_expected_fault_behaviour(self, peak_magnitude, fault_frequency):
        ...
        for harmonic in range(2, 20):
            augmentation = augmentation + self.exponential_decay(
                harmonic, fault_frequency
            ) * self.triangular_peak(peak_magnitude, fault_frequency * harmonic)
        return augmentation
