...
class PaderbornBearing:
    def __init__(self, name="PU", overlap=0.0, fault_events_per_segment=10):
        self.name = name
        ...
        }  # Technically some of the damages are artificial and others natural, but this is reported in paper.
    ...
    def get_fault_frequencies_for_rpm(self, rpm):
        """
        Calculate the expected fault frequencies for a specific RPM.

        :param rpm: RPM value
        :return: Dictionary with fault frequencies for different fault modes
        """
        # Convert bearing parameters to consistent units
        rd = self.bearing_params["rolling_element_diameter"]  # mm
        pd = self.bearing_params["pitch_diameter"]  # mm
        ...
        # Convert RPM to shaft frequency (Hz)
        fr = rpm / 60.0

        # Calculate fault frequency ratios
        rdpd = rd / pd  # roller diameter / pitch diameter ratio
        cs = np.cos(ca * np.pi / 180)  # cosine of contact angle
        ...
        return {
            "Outer Race": float(outer_race_freq),
            "Inner Race": float(inner_race_freq),
        }
    ...
    def calculate_segment_length(self):
        ...
        self.desired_segment_length = int(
            self.fault_events_per_segment * samples_per_event
        )
        ...
        print(
            f"  - Segment duration: {self.desired_segment_length / self.sampling_frequency:.3f} seconds"
        )
    ...
    def load_mat_file(self, file_path):
        ...
        try:
            ...
            for i in range(data_field.shape[1]):
                channel_name = (
                    names[0, i][0] if names[0, i].size > 0 else f"channel_{i}"
                )
    ...
    def load_healthy_data(self):
        ...
        for condition_dir in self.data_path.iterdir():
            if (
                ...
                and self.condition_mapping.get(condition_dir.name) == "healthy"
            ):
                ...
                for mat_file in mat_files:  # Process all files
                    ...
                    if (
                        operating_condition
                        and operating_condition in self.operating_conditions
                    ):
                        ...
                        if signal is not None:
                            ...
                            if segments.shape[0] > 0:
                                ...
                                healthy_data_by_condition[operating_condition].append(
                                    (segments, metadata)
                                )
    ...
    def load_faulty_data(self):
        ...
        for condition_dir in self.data_path.iterdir():
            if condition_dir.is_dir() and condition_dir.name in self.condition_mapping:
                ...
                if condition != "healthy":
                    print(
                        f"Loading faulty data from {condition_dir.name} -> {condition}"
                    )
                    ...
                    for mat_file in mat_files:  # Process all files
                        # Parse operating condition from filename
                        operating_condition = self.parse_operating_condition(
                            mat_file.name
                        )
                        if (
                            operating_condition
                            ...
                            if signal is not None:
                                ...
                                if segments.shape[0] > 0:
                                    if (
                                        operating_condition
                                        not in faulty_data_by_condition
                                    ):
                                        faulty_data_by_condition[
                                            operating_condition
                                        ] = {}
        ...
        for operating_condition in faulty_data_by_condition:
            for condition in faulty_data_by_condition[operating_condition]:
                fault_segments = faulty_data_by_condition[operating_condition][
                    condition
                ]
                if fault_segments:
    ...
    def segment_signal(self, signal):
        ...
        if n_segments == 0:
            print(
                f"  Warning: Signal too short ({signal_length} samples) for desired segment length ({segment_length} samples)"
            )
        ...
        print(
            f"  Segmented signal into {n_segments} segments of {segment_length} samples each"
        )
        return segments
...
def write_to_standard_structure():
    ...
    print(
        f"Loaded healthy data for {len(healthy_data_by_condition)} operating conditions:"
    )
    ...
    print(
        f"Loaded faulty data for {len(faulty_data_by_condition)} operating conditions:"
    )
    ...
    all_conditions = set(healthy_data_by_condition.keys()) | set(
        faulty_data_by_condition.keys()
    )
    ...
    for operating_condition in all_conditions:
        ...
        dataset_name = (
            f"{dataset.name}_{operating_condition}_RPM{rpm}_T{torque}_F{force}"
        )
        ...
        if combined_healthy is not None:
            # Calculate fault frequencies for the specific operating condition
            fault_frequencies = dataset.get_fault_frequencies_for_rpm(rpm)
            ...
            condition_metadata.update(
                {
                    ...
                    "expected_fault_frequencies": fault_frequencies,
                }
            )
            ...
            print(f"  Expected fault frequencies: {fault_frequencies}")
        else:
            print(f"  Skipping {operating_condition} - no healthy data found")

    print("Paderborn bearing datasets exported successfully")
