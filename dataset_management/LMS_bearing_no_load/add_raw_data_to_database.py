...
from dataset_management.utils import (
    processing,
    create_noisy_copies_of_dataset,
    compute_features,
)
from dataset_management.utils.pul_data_from_db import get_data_from_db_and_save_to_file
...
class LMS(object):
    ...
    def __init__(self, percentage_overlap=0.5):
        ...
        min_events = 8
        self.cut_signal_length = (
            get_required_signal_length_for_required_number_of_events(
                mean_rpm, min_events_per_rev, self.sampling_frequency, min_events
            )
        )
    ...
    def get_accelerometer_signal(self, dataset_dict_key):
        ...
        mat = loadmat(
            str(path_to_mat_file), matlab_compatible=True, simplify_cells=True
        )
        accelerometer_signals = mat["Signal_3"]["y_values"][
            "values"
        ]  # Signal_3 is the accelerometer
        speed_signal = mat["Signal_1"]["y_values"][
            "values"
        ]  # Signal_1 is the speed signal
        speed_correction_factor = mat["Signal_1"]["y_values"]["quantity"][
            "unit_transformation"
        ]["factor"]

        n_events_per_rev = self.n_events_per_rev[
            self.datasets[dataset_dict_key]["mode"]
        ]

        signals_at_oc = []
        for oc, samples in enumerate(
            segments_to_extract[1:]
        ):  # Do not use the first segment at the lowest speed
            start_id = int(samples[0] * 1e6)
            end_id = int(samples[1] * 1e6)

            speed_samples = (
                speed_signal[start_id:end_id] * speed_correction_factor
            )  # In RPM
            ...
            for accelerometer in [
                1
            ]:  # TODO: Notice we are only using the first accelerometer
                # for accelerometer in [0, 1]:
                sig = accelerometer_signals[
                    :, accelerometer
                ]  # Numbering is not necessarily correct

                signals_at_oc.append(
                    {
                        "signal": sig[start_id:end_id],  # Use only the cut out segment
                        "rpm": median_speed,
                        "oc": oc,
                        ...
                        if n_events_per_rev is not None
                        else None,
                        "all_expected_fault_frequencies": {
                            k: v * median_speed / 60
                            for k, v in self.n_events_per_rev.items()
                            if v is not None
                        },
                        "snr": 0,
                    }
    ...
    def add_to_db(self, dataset_dict_key):
        ...
        for signal_at_oc in signals_at_oc:
            signal_segments = processing.overlap(
                signal_at_oc["signal"], self.cut_signal_length, self.percentage_overlap
            )
    ...
    def add_all_to_db(self):
        for key, val in tqdm(
            self.datasets.items()
        ):  # TODO: Notice use of last 10 seconds of data
            self.add_to_db(key)
    ...
    def plot_speed_signals(self):
        ...
        for key, val in tqdm(list(self.datasets.items())[0:1]):
            mat = loadmat(
                lms_path.joinpath(key), matlab_compatible=True, simplify_cells=True
            )
            ...
            window_size = 20000
            down_sampled_speed_signal = (
                speed_signal[0 : len(speed_signal) - len(speed_signal) % window_size]
