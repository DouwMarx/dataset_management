...
segment_length = (
    4096  #  2048 #  8192 # 4096  # 1024 # 4096 # 2048 # 1024 # 4096# 1024 # 2048
)

for vehicle in ["Mondeo", "Vectra"]:
    data_map = np.lib.format.open_memmap(
        data_dir.joinpath(vehicle + "_sounds.npy"), mode="r+"
    )
    ...
    normal_query = (meta_data["ComputeLeakageBin"] == 0) & (
        meta_data["ComputeWhistleBin"] == 0
    )
    whistle_query = (meta_data["ComputeLeakageBin"] == 0) & (
        meta_data["ComputeWhistleBin"] == 1
    )
    leakage_query = (meta_data["ComputeLeakageBin"] == 1) & (
        meta_data["ComputeWhistleBin"] == 0
    )
    ...
    normal_data = data_map[normal_indexes][:, : segment_length * n_blocks].reshape(
        -1, segment_length
    )
    whistle_data = data_map[whistle_indexes][:, : segment_length * n_blocks].reshape(
        -1, segment_length
    )
    leakage_data = data_map[leakage_indexes][:, : segment_length * n_blocks].reshape(
        -1, segment_length
    )
    ...
    normal_amplitude_spectrum_mean = np.mean(
        np.abs(normal_spectrum), axis=0, keepdims=True
    )
    ...
    normal_spectral_flatness = np.exp(
        np.mean(np.log(normal_spectrum), axis=-1)
    ) / np.mean(normal_spectrum, axis=2)
    whistle_spectral_flatness = np.exp(
        np.mean(np.log(whistle_spectrum), axis=-1)
    ) / np.mean(whistle_spectrum, axis=2)

    print(
        f"Normal spectral flatness: {np.mean(normal_spectral_flatness):.2f} +/- {np.std(normal_spectral_flatness):.2f}"
    )
    print(
        f"Whistle spectral flatness: {np.mean(whistle_spectral_flatness):.2f} +/- {np.std(whistle_spectral_flatness):.2f}"
    )
    ...
    if show_plots:
        ...
        for mode_name, mode_data, mode_color in zip(
            ["whistle", "leakage", "normal"],
            [whistle_meta_data, leakage_meta_data, normal_meta_data],
            ["red", "blue", "green"],
        ):
        ...
        for mode_name, mode_data, mode_color in zip(
            ["whistle", "leakage", "normal"],
            [whistle_data, leakage_data, normal_data],
            ["red", "blue", "green"],
        ):
            for example_index in np.random.choice(
                range(np.min(number_of_samples_per_mode)), n_examples
            ):
    ...
    export_data_to_file_structure(
        dataset_name="siemens_vc_nvh_{}".format(vehicle.lower()),
