...
class DynamicalSystem:
    ...
    def make_sine(self):
        ...
        return (
            np.sin((t_range + self.phase) * 2 * np.pi * self.freq) * self.amp
            + self.offset
            + self.drift * t_range
            + np.random.normal(size=n_features, scale=self.noise)
        )
...
class DynamicalSystemDataset:
    ...
    def make_data_for_different_severities(self, max_severity, mode):
        ...
        severity_dict = {}
        for degree, severity in enumerate(
            severities
        ):  # Note that the first severity is technically healthy
            sys_properties = self.default_sys_properties
            sys_properties[mode] = severity
            data_for_mode_at_severity = DynamicalSystem(
                sys_properties
            ).make_many_samples(self.n_samples_test)
    ...
    def make_healthy_data(self):
        """For different severities of the same failure mode, compute many samples for each severity"""
        healthy_data_dict = {}
        for train_test, sample_size in zip(
            ["train", "test"], [self.n_samples_train, self.n_samples_test]
        ):
            sys_properties = self.default_sys_properties
            healthy_data = DynamicalSystem(sys_properties).make_many_samples(
                sample_size
            )
    ...
    def make_data_for_different_failure_modes(self):
        ...
        for failure_mode in failure_modes:
            data_for_mode = self.make_data_for_different_severities(
                failure_mode_max_severity[failure_mode], failure_mode
            )
...
def main():
    d = DynamicalSystemDataset(
        n_samples_test=1000, n_samples_train=2000, n_severities=10
    ).make_data_for_different_failure_modes()
