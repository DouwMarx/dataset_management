...
class Process(object):
    def __init__(self, n_features):
        ...
        self.healthy_cov = np.eye(
            n_features
        )  # Can later be extended to have non-identity covariance, Notice that this implies independence between features and PCA should not have an effect.
        self.faulty_cov = np.eye(
            n_features
        )  # Can later be extended to have non-identity covariance, Notice that this implies independence between features and PCA should not have an effect.

        # Random generation of data
        self.fault_direction = np.random.uniform(
            -1, 1, n_features
        )  # Random "true" direction in which the fault progresses
        self.fault_direction = self.fault_direction / np.linalg.norm(
            self.fault_direction
        )  # normalize the fault direction
    ...
    def get_healthy_data(self, n_samples):
        return pd.DataFrame(
            np.random.multivariate_normal(
                self.healthy_mean, self.healthy_cov, size=n_samples
            )
        )
    ...
    def get_faulty_data_at_severity(self, severity, n_samples):
        return pd.DataFrame(
            np.random.multivariate_normal(
                self.healthy_mean + severity * self.fault_direction,
                self.faulty_cov,
                size=n_samples,
            )
        )
    ...
    def take_measurements(self, n_samples, severities=None):
        ...
        self.faulty_test = {
            severity: self.get_faulty_data_at_severity(severity, n_samples)
    ...
    def get_imprecise_fault_directions(self, correlation, n_directions_to_generate=10):
        ...
        def objective(x):
            ...
            return (
                np.dot(x / np.linalg.norm(x), self.fault_direction) - correlation
            ) ** 2

        directions = []
        for i in range(n_directions_to_generate):
            optimisation_startpoint = self.fault_direction + np.random.uniform(
                -1, 1, self.n_features
            )
            ...
            print(
                "Correlation with true fault direction: ",
                np.dot(norm_sol, self.fault_direction),
            )
            directions.append(norm_sol)
        return directions


def generate_random_expected_fault_directions(
    n_fault_directions,
    n_features,
    expected_fault_direction_for_true_mode,
    ...
    while n_directions_generated < n_fault_directions - 1:
        ...
        new_direction = np.random.normal(
            size=n_features
        )  # Use normally distributed data to avoid the corner effect.
        new_direction /= np.linalg.norm(new_direction)
        if max_cor_with_true_fault_direction is not None:
            if (
                np.dot(expected_fault_direction_for_true_mode, new_direction)
                < max_cor_with_true_fault_direction
            ):
        ...
        if n_directions_generated % 10 == 0:
            print(
                f"Data generation {n_directions_generated / n_fault_directions * 100:.0f}% complete"
            )
...
export_data_to_file_structure(
    ...
    metadata={
        "ground_truth_fault_direction": p.fault_direction.tolist(),
        "dataset_name": "linear_gaussian_shift",
    },
)
