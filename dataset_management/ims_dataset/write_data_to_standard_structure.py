...
class IMSTest(object):
    ...
    def __init__(
        self, path_to_measurement_campaign: pathlib.Path, channel_info, rapid=False
    ):
        self.folder_name = (
            path_to_measurement_campaign.name
        ...
        self.measurement_names = [
            channel_dict["measurement_name"] for channel_dict in self.channel_info
        ]

        self.labeled_measurement_names = [
            channel_dict["measurement_name"]
            for channel_dict in self.channel_info
            if channel_dict["mode"] is not None
        ]
        ...
        if self.rapid:
            self.measurement_paths = np.random.choice(
                self.measurement_paths, size=1000, replace=False
            )

        # Make sure the record numbers follow the correct time stamps for different samples.
        self.time_stamps = sorted(
            [
                datetime.strptime(path.name, "%Y.%m.%d.%H.%M.%S")
                for path in self.measurement_paths
            ]
        )
        self.record_numbers = {
            self.time_stamps[i]: str(i + 1) for i in range(len(self.time_stamps))
        }  # Notice 1-based indexing for record number
        ...
        self.ims_meta_data = {
            ...
            "expected_fault_frequencies": {
                fault_type: self.bearing_geom_obj.get_expected_fault_frequency(
                    fault_type, self.rotation_frequency
                )
    ...
    def get_artificial_severity_index(self, record_number, channel):
        ...
        if record_number <= healthy_start_index:
            ...
            return int(
                np.ceil(
                    10
                    * (record_number - healthy_end_index)
                    / (self.n_records - healthy_end_index)
                )
            )
    ...
    def read_file_as_df(self, file_path):
        ...
        if len(measurement.index) != self.n_samples_per_measurement:
            raise IndexError(
                "The number of samples in the file is different than expected: ".format(
                    len(measurement.index)
                )
            )
        return measurement
    ...
    def create_document_per_channel(self, filepath):
        ...
        for channel_id, channel_name in enumerate(
            dataframe_of_measurement_for_each_channel.columns
        ):
            measurement_for_channel = dataframe_of_measurement_for_each_channel[
                channel_name
            ].values
            doc = self.create_document(
                list(measurement_for_channel), channel_id, filepath
            )
    ...
    def create_document(self, time_series_data, channel_id, file_path):
        ...
        doc = {
            ...
            "ims_channel_number": str(
                int(channel_id + 1)
            ),  # IMS convention is 1-based channel numbering
    ...
    def identify_ground_truth_health_state(self):
        ...
        for measurement in self.labeled_measurement_names:
            ...
            iqr_kurtosis = measurement_kurtosis.quantile(
                0.75
            ) - measurement_kurtosis.quantile(0.25)
            ...
            thresh_ref_start = 1
            reference_start_idx = moving_median_kurtosis[
                moving_median_kurtosis
                < median_kurtosis + thresh_ref_start * iqr_kurtosis
            ].index[0]

            thresh_fault_start = 2
            # Get a point after the reference start point where the kurtosis is larger than the median + 2*iqr
            faulty_start_idx = moving_median_kurtosis[
                moving_median_kurtosis.index > reference_start_idx
            ][
                moving_median_kurtosis
                > median_kurtosis + thresh_fault_start * iqr_kurtosis
            ].index[0]

            # Make uncertain start 90% of the way between reference and faulty
            uncertain_start_idx = int(
                reference_start_idx + 0.9 * (faulty_start_idx - reference_start_idx)
            )
            ...
            print(
                "points of interest: ",
                points_of_interest,
                "for signal of length: ",
                len(measurement_kurtosis),
            )

            # Plot the kurtosis as a function of time
            fig.add_trace(
                go.Scatter(
                    x=measurement_time_steps, y=measurement_kurtosis, name=measurement
                )
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=[measurement_time_steps.iloc[0], measurement_time_steps.iloc[-1]],
                    y=[median_kurtosis, median_kurtosis],
            ...
            for i in range(1, 3):
                fig.add_trace(
                    go.Scatter(
                        x=[
                            measurement_time_steps.iloc[0],
                            measurement_time_steps.iloc[-1],
                        ],
                        y=[
                            median_kurtosis + i * iqr_kurtosis,
                            median_kurtosis + i * iqr_kurtosis,
                        ],
                        name=measurement + " {}x iqr".format(i),
            ...
            for point in points_of_interest:
                ...
                fig.add_trace(
                    go.Scatter(
                        x=[point, point],
                        y=[measurement_kurtosis.min(), measurement_kurtosis.max()],
                        line=dict(color="black", dash="dash"),
                        showlegend=False,
                    )
                )
            ...
            fig.write_html(
                "ims_test_{}_{}_split.html".format(self.folder_name[0], measurement)
            )
    ...
    def write_to_file(self, target_location=None):
        if target_location is None:
            target_location = pathlib.Path(
                "/home/douwm/projects/PhD/code/biased_anomaly_detection/data"
            )

        documents = Parallel(n_jobs=8)(
            delayed(self.create_document_per_channel)(path)
            for path in tqdm(self.measurement_paths)
        )
        # Flatten the list of lists
        documents = [item for sublist in documents for item in sublist]
        ...
        split_dict = json.load(
            open("ims_test_{}_split.json".format(self.folder_name[0]), "r")
        )  # Load the dictionary prescribing the reference, uncertain and faulty split
        ...
        for measurement in self.labeled_measurement_names:
            ...
            reference_start_idx, uncertain_start_idx, faulty_start_idx = np.array(
                split_dict[measurement]
            ).astype(int)
            ...
            faulty_time_signals = {
                mode_name: np.expand_dims(faulty_time_signals, axis=1)
            }
...
if __name__ == "__main__":
    from dataset_management.ims_dataset.experiment_meta_data import (
        channel_info,
        test_folder_names,
    )
