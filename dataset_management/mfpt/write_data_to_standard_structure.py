...
class MFPT(object):
    ...
    def __init__(self, name="MFPT", overlap=0.0, fault_events_per_segment=8):
        ...
        self.data_path = pathlib.Path(__file__).parent.joinpath(
        ...
        number_of_samples_per_event = int(
            sampling_frequency * time_duration_per_event
        )  # samples per event
        self.desired_segment_length = int(
            self.fault_events_per_segment * number_of_samples_per_event
        )  # samples for all fault events in a segment
    ...
    def segment_signal(self, signal, segment_length):
        ...
        print(
            f"Segmented signal into {n_segments} segments of length {segment_length} samples each."
        )
        print(
            f"Each segment should contain approximately {self.fault_events_per_segment} fault events."
        )
    ...
    def load_mat_file(self, file_path):
        ...
        if bearing["gs"].ndim > 2:
            raise ValueError(
                f"Signal in {file_path} has more than 1 channel. Expected a single channel signal."
            )
    ...
    def load_outer_race_fault_data(self):
        ...
        outer_race_path = self.data_path.joinpath(
            "2 - Three Outer Race Fault Conditions"
        )
        outer_race_files = [
            f for f in os.listdir(outer_race_path) if f.endswith(".mat")
        ]
...
def write_mfpt_to_standard_structure():
    ...
    for i, (healthy_data, healthy_metadata) in enumerate(baseline_data):
        ...
        metadata["expected_fault_frequencies"] = {
            mode: outer_race_ff for mode in faulty_data_dict.keys()
        }
