...
def plot_baseline_and_fault_data():
    ...
    print(
        "Note: This script only uses the baseline data and the first set of outer race fault data,"
    )
    ...
    for i, (baseline_segments, baseline_metadata) in enumerate(baseline_data):
        baseline_num = os.path.splitext(baseline_metadata["file_name"])[0].split("_")[
            -1
        ]
        ...
        for seg_idx in [0, 2]:  # Choose first and third segments
            if seg_idx < baseline_segments.shape[0]:
                ...
                plot_time_series(
                    segment,
                    f"Baseline {baseline_num} - Segment {seg_idx + 1} (270 lbs load)",
                    fs,
                    ax=axs[0],
                )
                ...
                plot_stft(
                    segment,
                    f"STFT - Baseline {baseline_num} - Segment {seg_idx + 1} (270 lbs load)",
                    fs,
                    ax=axs[1],
                )
                ...
                plt.savefig(
                    plot_dir.joinpath(
                        f"baseline_{baseline_num}_segment_{seg_idx + 1}.png"
                    )
                )
                plt.close()
    ...
    for i, (fault_segments, fault_metadata) in enumerate(fault_data):
        ...
        for seg_idx in [0, 2]:  # Choose first and third segments
            if seg_idx < fault_segments.shape[0]:
                ...
                plot_time_series(
                    segment,
                    f"Outer Race Fault {fault_num} - Segment {seg_idx + 1} (270 lbs load)",
                    fs,
                    ax=axs[0],
                )
                ...
                plot_stft(
                    segment,
                    f"STFT - Outer Race Fault {fault_num} - Segment {seg_idx + 1} (270 lbs load)",
                    fs,
                    ax=axs[1],
                )
                ...
                plt.tight_layout()
                plt.savefig(
                    plot_dir.joinpath(f"fault_{fault_num}_segment_{seg_idx + 1}.png")
                )
                plt.close()
    ...
    if baseline_data and fault_data:
        ...
        baseline_num = os.path.splitext(baseline_data[0][1]["file_name"])[0].split("_")[
            -1
        ]
        ...
        plot_time_series(
            baseline_segment,
            f"Baseline {baseline_num} (270 lbs load)",
            baseline_fs,
            ax=axs[0, 0],
        )
        ...
        plot_stft(
            baseline_segment,
            f"STFT - Baseline {baseline_num} (270 lbs load)",
            baseline_fs,
            ax=axs[1, 0],
        )
        ...
        plot_time_series(
            fault_segment,
            f"Outer Race Fault {fault_num} (270 lbs load)",
            fault_fs,
            ax=axs[0, 1],
        )
        ...
        plot_stft(
            fault_segment,
            f"STFT - Outer Race Fault {fault_num} (270 lbs load)",
            fault_fs,
            ax=axs[1, 1],
        )
        ...
        fig.suptitle(
            "Comparison of Baseline and Outer Race Fault Data (270 lbs load, 97,656 Hz sampling rate)",
            fontsize=16,
        )
