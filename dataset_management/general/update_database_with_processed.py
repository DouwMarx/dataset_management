...
def compute_features_from_time_series_doc(doc):
    ...
    signal = signal - np.mean(signal)  # De-mean the signal
    ...
    env = envelope(
        signal
    )  # The envelope of the signal can also be added to the computed features
    freq, mag, phase = env_spec(signal, fs=fs)

    envelope_time_series = {
        "envelope_time_series": list(env),
    ...
    envelope_spectrum = {
        "envelope_spectrum": {
            "freq": list(limit_frequency_components(freq)),
            "mag": list(
                limit_frequency_components(mag**2)
            ),  # Notice that the squared envelope is used
    ...
    def compute_features_from_time_series_doc(self, doc):
        ...
        envelope_spectrum = {
            "envelope_spectrum": {
                ...
                "mag": list(
                    limit_frequency_components(mag)
                ),  # Notice that we are using the squared envelope spectrum
                "phase": list(limit_frequency_components(phase)),
            },
        }
        computed_features = [envelope_time_series, envelope_spectrum]
...
def process_no_bandpass(db_to_act_on):
    ...
    bandpass = False
    to_apply = ProcessData(
        db_to_act_on, bandpass=bandpass
    ).compute_features_from_time_series_doc
    query = {"time_series": {"$exists": True}}  # Process the time data
    DerivedDoc(query, "raw", "processed", to_apply, db_to_act_on).update_database(
        parallel=False
    )
    return db["processed"]
