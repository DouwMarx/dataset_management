...
from dataset_management.utils.update_database import (
    new_derived_doc,
    new_docs_from_computed,
    DerivedDoc,
)
import torch
...
class Encoding:
    def __init__(self, model_query, db_to_act_on, augmented):
        ...
        print(
            "Computing encodings for {} models".format(
                self.db["model"].count_documents(model_query)
            )
        )
        ...
        self.torch_models = [
            torch.load(model["path"]).to("cpu") for model in self.torch_docs
        ]
...
def main(db_to_act_on):
    ...
    for source, augmented in zip(["processed", "augmented"], [False, True]):
        ...
        derived_doc_func = Encoding(
            models_to_use_query, db_to_act_on, augmented
        ).compute_encoding_from_doc

        query = {"envelope_spectrum": {"$exists": True}}
        DerivedDoc(
            query, source, "encoding", derived_doc_func, db_to_act_on
