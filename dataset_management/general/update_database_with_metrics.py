...
class EncodingMovementMetrics(object):
    def __init__(self, model_used, db_to_act_on):
        ...
        self.expected_failure_modes = self.db[
            "encoding"
        ].distinct(
            "mode", {"augmented": True}
        )  # What are the potential modes that have been accounted for in the augmentation.

        self.actual_failure_modes = [
            mode
            for mode in self.db["encoding"].distinct("mode", {"augmented": False})
            if mode is not None
        ]
        ...
        self.severe_augmented_data_examples_per_mode = {
            mode: self.get_severe_augmented_data_example(mode)
        ...
        self.severe_augmented_data_median_per_mode = {
            key: list(np.median(val, axis=0))
            for key, val in self.severe_augmented_data_examples_per_mode.items()
        }

        # Get the position of a severely damaged sample so arrows can be drawn in latent space
        measured_severe_encoding_per_mode = {
            mode: self.get_severe_measured_data_example(mode)
            for mode in self.actual_failure_modes
        }
        measured_severe_encoding_median = {
            key: np.median(val, axis=0)
            for key, val in measured_severe_encoding_per_mode.items()
        }
        ...
        for mode, example_a in self.severe_augmented_data_median_per_mode.items():
            for mode, example_b in self.severe_augmented_data_median_per_mode.items():
                summation += np.linalg.norm(
                    np.array([example_a]) - np.array([example_b])
                )
        distance_between_augmented_clusters = summation / 3

        distance_healthy_to_severe_augmented_per_mode = {
            key: np.linalg.norm(
                np.array(self.healthy_encoding_median)
                - np.array(self.severe_augmented_data_median_per_mode[key])
            )
        ...
        distance_healthy_to_severe_measured_per_mode = {
            key: np.linalg.norm(
                np.array(self.healthy_encoding_median)
                - np.array(measured_severe_encoding_median[key])
            )
        ...
        self.expected_failure_directions_per_mode = {
            mode: list(self.get_expected_failure_direction(mode))
            for mode in self.expected_failure_modes
        }

        # Get the healthy projections onto the failure directions
        self.healthy_projection_per_mode = {
            mode: self.get_projection_onto_failure_direction(
                self.healthy_encoding, self.expected_failure_directions_per_mode[mode]
            )
            for mode in self.expected_failure_modes
        }
        ...
        for mode in self.expected_failure_modes:
            ...
            self.fitted_healthy_projection_per_mode.update(
                {mode: {"mu": mu, "std": std}}
            )

        self.db["meta_data"].replace_one(
            {"_id": "meta_data"},
            {
                "_id": "meta_data",
    ...
    def check_latent_constraints(self):
        ...
        for combination in combinations_of_modes:
            print(
                ...
                combination,
                np.dot(
                    self.expected_failure_directions_per_mode[combination[0]],
        ...
        for combination in combinations_of_modes:
            print(
                ...
                combination,
                np.linalg.norm(
                    self.severe_augmented_data_median_per_mode[combination[0]]
                    - self.severe_augmented_data_median_per_mode[combination[1]]
                ),
            )
    ...
    def get_severe_augmented_data_example(self, expected_mode):
        ...
        severe_augmented_encodings = [
            enc["encoding"]
            for enc in self.db["encoding"].find(
                {
                    "augmented": True,
        ...
        if len(severe_augmented_encodings) == 0:
            ...
            print(
                "Making use of {} augmented samples for mode {}".format(
                    len(severe_augmented_encodings), expected_mode
                )
            )
    ...
    def get_severe_measured_data_example(self, expected_mode):
        ...
        if len(severe_measured_encodings) == 0:
            ...
            print(
                "Making use of {} severe measured samples for mode {}".format(
                    len(severe_measured_encodings), expected_mode
                )
            )
    ...
    def get_healthy_encoding_examples(self):
        ...
        all_healthy_encoding = [
            enc["encoding"]
            for enc in self.db["encoding"].find(
                {"model_used": self.model_used, "severity": 0, "augmented": False}
            )
        ]
        ...
        if len(all_healthy_encoding) == 0:
            ...
            print(
                "Making use of {} healthy samples defining the healthy distribution".format(
                    len(all_healthy_encoding)
                )
            )
    ...
    def get_expected_failure_direction(self, expected_mode):
        ...
        direction_healthy_to_augmented = (
            self.severe_augmented_data_median_per_mode[expected_mode]
            - self.healthy_encoding_median
        )
        normalized_direction_healthy_to_augmented = (
            direction_healthy_to_augmented
            / np.linalg.norm(direction_healthy_to_augmented)
        )
        return normalized_direction_healthy_to_augmented

    def get_projection_onto_failure_direction(
        self, measured_encoding, normalized_direction_healthy_to_augmented
    ):
        measured_projection = np.dot(
            measured_encoding, normalized_direction_healthy_to_augmented
        )
        # healthy_projection = np.dot(self.healthy_encoding, normalized_direction_healthy_to_augmented)
        return measured_projection

    def get_metrics_for_failure_direction_projection(
        self, measured_encoding, expected_mode
    ):
        measured_projection = self.get_projection_onto_failure_direction(
            measured_encoding, self.expected_failure_directions_per_mode[expected_mode]
        )
    ...
    def compute_metrics_from_doc(self, doc):
        ...
        for expected_mode in self.expected_failure_modes:
            (
                measured_projection,
                p,
            ) = self.get_metrics_for_failure_direction_projection(
                measured_encoding, expected_mode
            )
...
def main(db_to_act_on):
    ...
    for model_used in db["encoding"].distinct("model_used"):
        ...
        }  # Applying metrics only to non-augmented data
        em_obj = EncodingMovementMetrics(model_used, db_to_act_on)
        DerivedDoc(
            query, "encoding", "metrics", em_obj.compute_metrics_from_doc, db_to_act_on
