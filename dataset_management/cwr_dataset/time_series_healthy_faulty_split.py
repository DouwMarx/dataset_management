...
df = get_cwru_data_frame(
    min_average_events_per_rev=min_average_events_per_rev,
    overlap=overlap,
...
for measurement_location in measurement_locations:  # Make 4 separate datasets
    for sampling_rate in unique_sampling_rates:
        ...
        for i, row in sub_data_sets.iterrows():
            ...
            fault_state = (
                0 if row["Fault Location"] == "NONE" else 1
            )  # Healthy vs Faulty
            ...
            n_signals = (
                n_signals - 2
            )  # Remove the signal between train and val and val and test to avoid leakage
        ...
        dataset_df.to_pickle(
            processed_dir.joinpath(
                f"{measurement_location}_{sampling_rate}kHz_{min_average_events_per_rev}evperrev_{overlap}ovrlp_binary-all_fault_modes_speed_fault_location.pkl"
            )
        )
