...
from dataset_management.utils.time_frequency import (
    get_required_signal_length_for_required_number_of_events,
    get_number_of_fault_events_for_segment_length,
)
from dataset_management.utils.write_data_in_standard_format import (
...
def get_cwru_meta_data_from_csv():
    ...
    meta_data = pd.read_csv(
        this_file.parent.joinpath("meta_data_table.csv"), dtype=str
    ...
    id_cols = [
        col for col in meta_data.columns if col not in value_cols
    ]  # The columns that are not fault modes
...
class CWR(object):
    ...
    def __init__(
        self,
        required_average_number_of_events_per_segment=30,
        name="CWR",
        data_path=cwr_path,
        overlap=0.0,
    ):
        ...
        if overlap > 0:
            ...
            warnings.warn(
                "Do not use randomized splitting when overlap is used. This will cause data leakage. Instead, use a fixed split and discard the overlap."
            )
        ...
        rpms = np.unique(
            self.sample_labels["Shaft speed [rpm]"].values.astype(int)
        )  # Unique RPMs
        ...
        self.cut_signal_length_for_12k = get_required_signal_length_for_required_number_of_events(
            mean_rpm,
        ...
        print(
            "Signals cut to length: {}, ensuring that there are at least {} events per revolution".format(
                self.cut_signal_length_for_12k,
                required_average_number_of_events_per_segment,
            )
        )
        print("")

        worst_case_faults_per_segment = get_number_of_fault_events_for_segment_length(
            min(rpms),
            minimum_number_of_events_occurring_per_revolution,
            12000,
            self.cut_signal_length_for_12k,
        )
        best_case_faults_per_segment = get_number_of_fault_events_for_segment_length(
            max(rpms),
            minimum_number_of_events_occurring_per_revolution,
            12000,
            self.cut_signal_length_for_12k,
        ...
        print(
            "Worst case  and best case fault events per segment: ",
            worst_case_faults_per_segment,
            best_case_faults_per_segment,
        )

        self.n_faults_per_revolution = {
            mode: self.get_expected_fault_frequency_for_mode(mode, 60)
        ...
        self.dataset_meta_data = {
            ...
            "channel_names": [
                ...
                "FE",
                "BA",
            ],  # Measurements are taken at the drive end, fan end and base
            "_id": "meta_data",
        }
    ...
    def get_label(self, file_stem):
        ...
        sample_meta_data = (
            self.sample_labels[self.sample_labels["File Number"] == file_stem]
            .iloc[0]
            .to_dict()
        )
        ...
        if "ball" in sample_meta_data["Fault Mode"].lower():
            expected_fault_frequency = self.get_expected_fault_frequency_for_mode(
                "ball", shaft_speed
            )
        ...
        derived_meta_data = {}  # TODO: Add derived meta data like expected/approximate fault frequencies etc.
    ...
    def segment_signals(self, signal, fs_is_48KHz=False):
        if fs_is_48KHz:
            cut_length = (
                self.cut_signal_length_for_12k * 4
            )  # Cut signal length was computed for 12KHz data, so we need to multiply by 4 to get the correct length for 48KHz data
        else:
            cut_length = self.cut_signal_length_for_12k

        step_size = int(
            cut_length * (1 - self.overlap)
        )  # step size derived from overlap
        n_segments = (
            len(signal) - cut_length
        ) // step_size + 1  # number of segments considering overlap

        segments = np.zeros(
            (n_segments, 1, cut_length)
        )  # initialize array to store segments (Notice channel dimension is 1 for torch)
    ...
    def get_data_per_channel_from_mat_file(self, file_number):
        sample_meta_data, derived_meta_data = self.get_label(
            file_number
        )  # First load the meta data from the CSV file
        ...
        present_measurement_locations = [
            key
            for key in mat_file.keys()
            if any(
                channel in key for channel in self.dataset_meta_data["channel_names"]
            )
        ]  # Channel names as in the mat file
        present_measurement_location_names = [
            channel
            for channel in self.dataset_meta_data["channel_names"]
            if any(channel in key for key in mat_file.keys())
        ...
        for measurement_location, measurement_location_name in zip(
            present_measurement_locations, present_measurement_location_names
        ):
            ...
            if (
                measurement_location.split("X")[1].split("_")[0]
                != sample_meta_data["File Number"]
            ):
                print(
                    "Note: channel name in mat file does not match dataset number for file: "
                    + str(file_number)
                    + " channel: "
                    + str(measurement_location)
                    + " file_number: "
                    + str(sample_meta_data)
                )
            ...
            if sampling_rate_associated_with_file == "48":
                data = self.segment_signals(
                    signal, fs_is_48KHz=True
                )  # Cut into segments
            else:
                data = self.segment_signals(signal, fs_is_48KHz=False)

            channel_specific_meta_data = sample_meta_data.copy()
            channel_specific_meta_data.update(
                {"Measurement Location": measurement_location_name}
            )
            ...
            if sample_meta_data["Fault Mode"] == "Reference":
                ...
                channel_specific_meta_data_ref.update(
                    {"Sampling Rate [kHz]": "12"}
                )  # Update the sampling rate to 12 since it was downsampled
                ...
                datasets.append(
                    (data_ds, channel_specific_meta_data_ref, derived_meta_data)
                )
        return datasets
    ...
    def load_all_mat_files(self):
        ...
        for file_name in self.sample_labels[
            "File Number"
        ].values:  # Loop through all files measured files
            ...
            ]  # Datasets were saved in tuples of (data, channel_meta_data, derived_meta_data)
...
def get_data_entry(labels, signals, operating_condition, severity, mode, channel):
    ...
    if len(data_entry) == 0:
        print(
            "No data found for operating condition: "
            + str(operating_condition)
            + " and severity: "
            + str(severity)
            + " and mode: "
            + str(mode)
        ...
        print(
            "Multiple data entries found for operating condition: oc"
            + str(operating_condition)
            + " severity: "
            + str(severity)
            + " mode: "
...
def write_cwru_to_standard_file_structure(min_average_events_per_rev):
    ...
    operating_condition_parameters = [
        "Motor load [hp]",
        "Sampling Rate [kHz]",
    ...
    for (
        index,
        healthy_data_row,
    ) in all_reference_datasets.iterrows():  # Loop through all the reference data and find the corresponding faulty data for different fault conditions
        ...
        corresponding_faulty_data = all_faulty_datasets[
            (
                all_faulty_datasets["Sampling Rate [kHz]"]
                == healthy_data_row["Sampling Rate [kHz]"]
            )
            & (
                all_faulty_datasets["Motor load [hp]"]
                == healthy_data_row["Motor load [hp]"]
            )
            & (
                all_faulty_datasets["Measurement Location"]
                == healthy_data_row["Measurement Location"]
            )
        ]
        ...
        for index, faulty_data_row in corresponding_faulty_data.iterrows():
            ...
            # Faulty names
            faulty_name = "_".join(
                key + "_" + faulty_data_row[key] for key in fault_condition_parameters
            )
...
def get_cwru_data_frame(
    min_average_events_per_rev, overlap, path_to_write=None, data_path=cwr_path
):
    # Writes the data to the standard file structure
    cwr_data = CWR(
        required_average_number_of_events_per_segment=min_average_events_per_rev,
...
if __name__ == "__main__":
    ...
    write_cwru_to_standard_file_structure(
        min_average_events_per_rev
    )  # If you want folders with each operating condition
