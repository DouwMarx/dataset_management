...
def get_cwru_meta_data_from_csv():
    ...
    # This table is also directly available from the website of the source of the dataset
...
class CWR(object):
    ...
    def __init__(self, required_average_number_of_events_per_rev=30, name="CWR"):
        ...
        # Find the mean RPM so that we can compute the required signal length to ensure that there are at least the required number of fault events per sample
        ...
        self.cut_signal_length = get_required_signal_length_for_required_number_of_events(mean_rpm,
                                                                                          ...
                                                                                          12000,
                                                                                          # TODO: The sampling rate is hard-coded and does not apply to the datasets sampled at a higher rate.
    ...
    def down_sample_reference(self, signal):
        # The reference condition was sampled only at 48KHz (not 12KHz), need to downsample to be able to compare with 12KHz faulty data
        downsampling_factor = 4
        # Downsample the filtered signal by the factor of 4, ensuring that an aliasing filter is applied
    ...
    def get_data_per_channel_from_mat_file(self, file_number):
        ...
        datasets = []  # Each file might contrain multiple channels, so we loop through all channels and add each as a "dataset"
        for channel, channel_name in zip(present_channels, present_channel_names):
            ...
            signal = mat_file[channel].flatten()
...
signals, labels = cwr_data.load_all_mat_files()

# The remainder of the code writes the data to the standard file structure

all_reference_datasets = labels[labels["Fault Location"] == "NONE"]
all_faulty_datasets = labels[labels["Fault Location"] != "NONE"]  # notice the negation
