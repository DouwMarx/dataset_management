...
def cut_into_non_overlapping_segments(signal, segment_length=1000):
    ...
    segments = signal[: num_segments * segment_length].reshape(
...
def get_data(
    ...
    # Validate stationarity parameter
    if stationarity not in ["stationary", "non_stationary"]:
        raise ValueError("stationarity must be either 'stationary' or 'non_stationary'")
    stationarity_suffix = "1" if stationarity == "stationary" else "2"

    # Validate condition parameter
    if condition not in ["normal", "faulty"]:
        raise ValueError("condition must be either 'normal' or 'faulty'")
    config_num = (
        "2" if condition == "normal" else "4"
    )  # config 2 for normal, 4 for faulty

    dataset_path = pathlib.Path(dataset_path)

    data_path = (
        f"desir_ii_configuration_{config_num}_{condition}_{stationarity_suffix}.mat"
    )
    performance_path = (
        f"desir_ii_config_{config_num}_perfo_{condition}_{stationarity_suffix}.mat"
    )
    ...
    speed_rpm = performance[
        "NL"
    ].flatten()  # Use the low pressure rotor speed (Connected to the fan) since this is the relative rotational speed relative to the staionary OGV blades following directly after the fan
    ...
    measurement_segments = cut_into_non_overlapping_segments(
        full_measurement, segment_length
    )
...
if __name__ == "__main__":
    ...
    faulty_data = {"ogv_blades": faulty_segments}
    ...
    export_data_to_file_structure(
        dataset_name="DGEN380_" + channel + "_" + stationarity,
