...
def make_plots_for_all_data():
    ...
    with PdfPages(
        f"all_plots_segment_length_{segment_length}_rpm_measured_at_fan_directly_{rpm_measured_at_fan_directly}.pdf"
    ) as pdf:
        for channel in channels:
            for stationarity in stationarities:
                ...
                fig.suptitle(
                    f"Channel: {channel}, Stationarity: {stationarity}, Segment Length {segment_length}",
                    fontsize=16,
                )

                for health_state, health_state_name in zip(
                    ["normal", "faulty"], data_store.keys()
                ):
                    data = get_data(
                        health_state,
                        stationarity,
                    ...
                    for order_name, order_value in fault_orders.items():
                        sp_method = SimpleDemodulatedEnergySP(
                            target_order=order_value, fs=fs, mean_rpm=mean_rpm
                        )
                        indicators = sp_method.get_test_statistic(segments)
                        data_store[health_state_name]["indicators"][order_name] = (
                            indicators
                        )

                    # Store example segment
                    data_store[health_state_name]["example_segment"] = segments[
                        0
                    ]  # Example segment
                ...
                for order_name, order_value in fault_orders.items():
                    y_true = np.concatenate(
                        [
                            np.zeros(
                                len(data_store["Normal"]["indicators"][order_name])
                            ),
                            np.ones(
                                len(data_store["Faulty"]["indicators"][order_name])
                            ),
                        ]
                    )
                ...
                best_order = max(
                    roc_auc_values_for_different_orders.items(), key=lambda x: x[1]
                )[0]
                for health_state_name, data in data_store.items():
                    axs[1, 0].plot(
                        data["indicators"][best_order],
                        label=f"{health_state_name}",
                        color=order_colors[best_order]
                        if health_state_name == "Normal"
                        else "gray",
                    )
                axs[1, 0].set_title(
                    f"Indicator Trace - {best_order} (Order {fault_orders[best_order]}, Best AUC = {roc_auc_values_for_different_orders[best_order]:.3f})"
                )
                ...
                axs[1, 1].hist(
                    data_store["Normal"]["indicators"][best_order],
                    bins=20,
                    alpha=0.5,
                    label="Normal",
                    color=order_colors[best_order],
                ...
                axs[1, 1].hist(
                    data_store["Faulty"]["indicators"][best_order],
                    bins=20,
                    alpha=0.5,
                    label="Faulty",
                    color="gray",
                ...
                axs[1, 1].set_title(
                    f"Histogram of Indicators - {best_order} (Order {fault_orders[best_order]}, Best AUC = {best_auc:.3f})"
                )
                ...
                axs[2, 0].plot(
                    time,
                    data_store["Normal"]["example_segment"],
                    label="Normal Segment",
                )
                axs[2, 0].plot(
                    time,
                    data_store["Faulty"]["example_segment"],
                    label="Faulty Segment",
                    alpha=0.7,
                )
                ...
                for (order_name, order_value), color in zip(
                    fault_orders.items(), fault_colors
                ):
                ...
                axs[2, 1].plot(
                    freqs, faulty_fft, label="Faulty FFT", color="red", alpha=0.7
                )
                ...
                axs[3, 0].plot(
                    f_faulty, Pxx_faulty, label="Faulty PSD", color="red", alpha=0.7
                )
                ...
                current_plot = (
                    channels.index(channel) * len(stationarities)
                    + stationarities.index(stationarity)
                    + 1
                )
                print(
                    "Percentage done: {:.2f}%".format(
                        100 * current_plot / total_plots_to_make
                    )
                )
        ...
        auc_columns = [col for col in performance_df.columns if col.startswith("AUC_")]
        mean_aucs = {
            col.replace("AUC_", ""): performance_df[col].mean() for col in auc_columns
        }
