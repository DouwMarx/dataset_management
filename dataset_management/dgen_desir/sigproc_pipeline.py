...
class SP(object):
    ...
    def get_test_statistic(self, measurement, **kwargs):
        ...
        demodulated_signal = self.demodulate_at_target_order(
            measurement, theta_vec, self.target_order
        )
        energy_at_target_order = np.abs(
            np.mean(demodulated_signal, axis=-1)
        )  # Get magnitude of complex valued
    ...
    def demodulate_at_target_order(self, signal, theta_vec, target_order):
        ...
        """
        return (
            signal * np.exp(-1j * 2 * np.pi * target_order * theta_vec)
        )  # Like normal Fourier transform as a function of revolutions, rather than seconds
...
class NoAngularResampleSP(SP):
    ...
    def get_signal_as_function_of_revolutions(self, measurement):
        ...
        if measurement.shape[1] > 1:
            print(
                "Warning: No angular resampling is done. The second channel is ignored."
            )
        ...
        n_total_revolutions = (
            self.mean_speed * t_duration / 60
        )  # (revs / min) * (min / s) = revs
        theta_vec = np.linspace(0, n_total_revolutions, measurement.shape[-1])  # [revs]

        # Make it match shape (batch, 1, samples)
        theta_vec = np.repeat(theta_vec, measurement.shape[0]).reshape(
            measurement.shape[0], 1, measurement.shape[-1]
        )
...
class AngularResampleSP(SP):
    ...
    def get_theta_as_function_of_time(self, instantaneous_speed_rpm):
        ...
        """
        instantaneous_speed_rps = (
            instantaneous_speed_rpm / 60
        )  # [revolutions per second]
        theta = (
            np.cumsum(instantaneous_speed_rps, axis=-1) / self.fs
        )  # (revs / s) * (s/1) = revs
        return theta

    def get_signal_as_function_of_revolutions(self, measurement):
        signal_of_t = measurement[:, 0, :]  # The first channel is the signal
        instantaneous_speed = measurement[
            :, 1, :
        ]  # The second channel is the speed measurement

        theta_of_t = self.get_theta_as_function_of_time(instantaneous_speed)

        # Do angular resampling
        interp_model = interp.interp1d(
            theta_of_t, signal_of_t, kind="linear", fill_value="extrapolate"
        )

        chosen_thetas = np.arange(
            np.min(theta_of_t),
            np.max(theta_of_t),
            1 / self.desired_samples_per_revolution,
        )
        signal_of_theta = interp_model(
            chosen_thetas
        )  # Evaluate the interpolation model at the chosen thetas
...
class SimpleDemodulatedEnergySP(object):
    ...
    def simple_demodulated_energy(self, measurement):
        t_duration = measurement.shape[-1] / self.fs  # samples / (samples / s) = s
        n_total_revolutions = (
            self.mean_rpm * t_duration / 60
        )  # (revs / min) * (min / s) = revs
        theta_vec = np.linspace(0, n_total_revolutions, measurement.shape[-1])  # [revs]

        demodulated_signal = measurement * np.exp(
            -1j * 2 * np.pi * self.target_order * theta_vec
        )
        energy_at_target_order = np.abs(
            np.mean(demodulated_signal, axis=-1)
        )  # Get magnitude of complex valued
...
if __name__ == "__main__":
    ...
    for operating_regime, health_state in zip(
        ["stationary", "non_stationary"], ["normal", "faulty"]
    ):
        constant_speed = 20  # RPM
        if operating_regime == "stationary":
            rpm_as_function_of_time = (
                np.ones((n_measurements, 1, n_samples)) * constant_speed
            )
            # Define the SP object
        else:
            n_components = 4
            random_amplitudes = np.random.uniform(
                0, 1, (n_measurements, n_components)
            ).reshape(n_measurements, n_components, 1)
            random_frequencies = np.random.uniform(
                1, 5, (n_measurements, n_components)
            ).reshape(n_measurements, n_components, 1)
            random_phases = np.random.uniform(
                0, 2 * np.pi, (n_measurements, n_components)
            ).reshape(n_measurements, n_components, 1)
            random_offsets = np.random.uniform(
                0.9 * constant_speed, 1.1 * constant_speed, (n_measurements, 1)
            ).reshape(n_measurements, 1, 1)

            rpm_as_function_of_time = (
                np.sum(
                    random_amplitudes
                    * np.sin(random_frequencies * time_vector + random_phases),
                    axis=1,
                    keepdims=True,
                )
                + random_offsets
        ...
        rps_as_function_of_time = (
            rpm_as_function_of_time / 60
        )  #  (revolutions per minute) * (minutes / second) = revolutions per second
        thetas = (
            np.cumsum(rps_as_function_of_time, axis=-1) / fs
        )  # (revolutions per second) * (seconds / sample) = revolutions per sample
        fault_component = np.sin(
            2 * np.pi * fault_order * thetas
        )  # The fault component
        ...
        for i in range(subset_to_plot):
            plt.plot(
                time_vector[0, 0, :],
                rpm_as_function_of_time[i, 0, :],
                label="Measurement {}".format(i),
            )
        ...
        normal_test_statistic = simple_demodulated_energy(
            normal_signal, target_order=fault_order, fs=fs, mean_rpm=constant_speed
        )
        faulty_test_statistic = simple_demodulated_energy(
            faulty_signal, target_order=fault_order, fs=fs, mean_rpm=constant_speed
        )

        # Calculate deflection coefficient
        deflection_coefficient = np.mean(faulty_test_statistic) - np.mean(
            normal_test_statistic
        )
        print(
            "Deflection coefficient for {} regime: {}".format(
                operating_regime, deflection_coefficient
            )
        )
