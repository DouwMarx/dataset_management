...
class CZT(object):
    ...
    def __init__(
        self, percentage_overlap=0.5, required_average_number_of_events_per_rev=30
    ):
    ...
    def load_time_domain_data(self):
        ...
        export_data_to_file_structure(
            ...
            faulty_data_dict={
                key: value
                for key, value in data_for_each_class.items()
                if key != "healthy"
            },
    ...
    def split_data_in_classes(self, data):
        ...
        freqs = np.fft.rfftfreq(
            len(healthy_example), d=1 / 20480
        )  # Sampling freq listed as 20kHz but more likely to be 20480 Hz?
        ...
        for fig, name in zip(
            [fig_freq, fig_time], ["freq_domain_examples", "time_domain_examples"]
        ):
        ...
        data_for_each_class = {
            key: np.expand_dims(value, axis=1)
            for key, value in data_for_each_class.items()
        }
